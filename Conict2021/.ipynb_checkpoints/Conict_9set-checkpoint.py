...
def threshold_image(src,thresh): #Thresh hold da imagem para o métodod Canny, utilizando THRESH_BINARY
    _,bin = cv.threshold(src,thresh,255,cv.THRESH_BINARY)
    return bin

def threshold_image_inv(src,thresh): #Thresh hold da imagem para o métodod Watershed, utilizando THRESH_BINARY_INV
...
def process_image(src,percent): #Método de processamento da imagem para Watershed
    kernel = np.ones((3,3),np.uint8) #Blur com kernel 3X3
    closing = cv.morphologyEx(src,cv.MORPH_CLOSE,kernel, iterations = 1) #Utilizado para fechar os possíveis buracos da imagem
    bg = cv.dilate(closing,kernel,iterations=3) #Dilatar os objetos para identificar o background
    dist = cv.distanceTransform(bg,cv.DIST_L2,3) #Calcula a distância dos pixels 0 (preto) mais perto de cada pixel na imagem
    _,fg = cv.threshold(dist,percent*dist.max(),255,0) #Threshold utilizando determinada porcentagem da distância maxima de cada pixel encontrando em "dist" como retorno
    fg = np.uint8(fg) #Truncamento de fg para utin8
...
percentList = [0.2,0.2,0.1,0.3,0.3,0.1,0.1,0.2,0.4,0.2] #Por ser a porcentagem da distância máxima dos pixels 0 para os não zero, quanto maior a porcentagem, menos contorno haverá.
