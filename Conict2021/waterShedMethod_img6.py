...
def show(fileName,thresh,percent):
    ...
    dist = cv.distanceTransform(bg,cv.DIST_L2,5)
    ...
    cv.imshow("Bin"+fileName,bin)
    cv.imshow("Closing"+fileName,closing)
    cv.imshow("bg"+fileName,bg)
    cv.imshow("dist"+fileName,dist)
    cv.imshow("fg"+fileName,fg)
    cv.imshow("unknown"+fileName,unknown)
